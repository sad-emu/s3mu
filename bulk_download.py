...
def main():
    ...
    for page in paginator.paginate(Bucket=args.bucket_name):
        for obj in page.get('Contents', []):
            print(f"Processing {obj['Key']}...")
            key = obj['Key']
            output_path = os.path.join(args.output_path, os.path.relpath(key))
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            download.raw_multipart_download(args.bucket_name, key, output_path, part_size, total_size=obj['Size'], s3=s3)
